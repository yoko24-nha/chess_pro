...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _setPlayerNameDialog() async {
    ...
    final result = await showDialog<String?>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(controller: controller, decoration: const InputDecoration(hintText: 'Enter your display name')),
        actions: [
  ...
  Future<void> _createRoom() async {
    ...
    final result = await showDialog<int?>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: const InputDecoration(hintText: 'Minutes per player (leave blank for no clock)'),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(null), child: const Text('Cancel')),
          ElevatedButton(onPressed: () => Navigator.of(ctx).pop(int.tryParse(minutesController.text ?? '0')), child: const Text('Create')),
        ],
      ),
    );
    ...
    final id = await _fsService.createRoom(_controller.getFen(), _playerName, timePerPlayer: timePerSec);
    setState(() {
      _roomId = id;
      if (timePerSec != null) {
  ...
  Future<String?> _showJoinDialog() async {
    ...
    final result = await showDialog<String?>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(controller: controller, decoration: const InputDecoration(hintText: 'Enter Room ID')),
        actions: [
          TextButton(onPressed: () => Navigator.of(ctx).pop(null), child: const Text('Cancel')),
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.of(ctx).pop(true), child: const Text('Đầu hàng')),
        ],
      ),
    );
  ...
  void _listenRoom(String roomId) {
    ...
    _roomSub = _fsService.listenRoom(roomId, (data) {
      ...
      final String serverTurn = (data['turn'] is String) ? data['turn'] as String : _currentTurn;
      ...
      // log for debugging
      debugPrint('[listenRoom] serverTurn=$serverTurn lastUpdate=${lastUpdate?.toDate().toIso8601String()} players=${players.length}');
      debugPrint('[listenRoom] serverWhite=$whiteRemFromServer serverBlack=$blackRemFromServer adjustedW=$adjustedWhite adjustedB=$adjustedBlack');
      ...
      if (timeoutWinner != null) {
        if (mounted) {
          final text = timeoutWinner == 'white' ? 'bên trắng thắng (timeout)' : 'bên đen thắng (timeout)';
      ...
      if (_timePerPlayer > 0 && _timeoutWinner == null && _surrenderedBy == null && lastUpdate != null && playersCount >= 2) {
        _startClockTicker();
      } else {
        _clockTimer?.cancel();
      }
  ...
  void _startClockTicker() {
    ...
    debugPrint('[startClockTicker] start; currentTurn=$_currentTurn white=$_whiteRemaining black=$_blackRemaining');
    ...
    _clockTimer = Timer.periodic(const Duration(seconds: 1), (timer) async {
      ...
      // debug each tick
      debugPrint('[tick] nowTurn=$nowTurn white=$_whiteRemaining black=$_blackRemaining');
      ...
      if (_whiteRemaining <= 0 || _blackRemaining <= 0) {
        ...
        if (mounted) {
          final text = winner == 'white' ? 'bên trắng thắng (timeout)' : 'bên đen thắng (timeout)';
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(text)));
        }
        _clockTimer?.cancel();
      }
        ...
        child: Column(
          children: [
            UserAccountsDrawerHeader(
              ...
              currentAccountPicture: CircleAvatar(child: Text((_playerName.isNotEmpty ? _playerName[0] : '?').toUpperCase())),
            ),
            ListTile(leading: const Icon(Icons.person), title: const Text('Set Name'), onTap: () { Navigator.of(context).pop(); _setPlayerNameDialog(); }),
            ListTile(leading: const Icon(Icons.add_box_outlined), title: const Text('Create Room'), onTap: () { Navigator.of(context).pop(); _createRoom(); }),
            ListTile(leading: const Icon(Icons.meeting_room), title: const Text('Join Room'), onTap: () async {
              Navigator.of(context).pop();
              final input = await _showJoinDialog();
              if (input != null && input.isNotEmpty) { _joinController.text = input; _joinRoom(); }
            }),
            ...
            ListTile(leading: const Icon(Icons.info_outline), title: const Text('About'), onTap: () {
              Navigator.of(context).pop();
              showAboutDialog(context: context, applicationName: 'Flutter Chess Starter', applicationVersion: '0.1', children: const [Text('Starter app with Firestore sync.')]);
            }),
          ],
        ),
      ),
  ...
  Widget _playersWidget() {
    ...
    return SizedBox(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(children: _playersInRoom.map((p) => Padding(padding: const EdgeInsets.only(right: 8.0), child: Chip(label: Text(p)))).toList()),
      ),
    );
  }
                  ...
                  children: [
                    ...
                    if (_timePerPlayer > 0)
                      Row(
                        ...
                        children: [
                          Column(children: [
                            Text('White', style: TextStyle(fontWeight: _currentTurn == 'white' ? FontWeight.bold : FontWeight.normal)),
                            Text(_formatTime(_whiteRemaining), style: const TextStyle(fontSize: 18, fontFamily: 'monospace')),
                          ]),
                          Column(children: [
                            Text('Black', style: TextStyle(fontWeight: _currentTurn == 'black' ? FontWeight.bold : FontWeight.normal)),
                            Text(_formatTime(_blackRemaining), style: const TextStyle(fontSize: 18, fontFamily: 'monospace')),
                          ]),
                        ],
                      ),
                    if (_timeoutWinner != null)
                      Padding(
                        ...
                        child: Text(_timeoutWinner == 'white' ? 'bên trắng thắng (timeout)' : 'bên đen thắng (timeout)', style: const TextStyle(color: Colors.red)),
                      ),
