...
class FlutterChessWidget extends StatefulWidget {
  ...
  final bool? autoOrientation; // Tự động xoay bàn cờ dựa trên player color
  final bool? isPlayerWhite; // Người chơi đang cầm quân trắng?
...
class _FlutterChessWidgetState extends State<FlutterChessWidget> {
  ...
  void initState() {
    ...
    // Nếu autoOrientation được bật và có thông tin về player color
    if (widget.autoOrientation == true && widget.isPlayerWhite != null) {
      _isWhiteOrientation = widget.isPlayerWhite!;
    }
    widget.controller.addListener(_onControllerChanged);
  }

  @override
  void didUpdateWidget(FlutterChessWidget oldWidget) {
    super.didUpdateWidget(oldWidget);
    // Cập nhật orientation khi player color thay đổi
    if (widget.autoOrientation == true && widget.isPlayerWhite != null) {
      if (oldWidget.isPlayerWhite != widget.isPlayerWhite) {
        setState(() {
          _isWhiteOrientation = widget.isPlayerWhite!;
        });
      }
    ...
    return Column(
      children: [
        ...
        Padding(
          ...
          child: Row(
            children: [
              if (widget.autoOrientation != true) ...[
                ElevatedButton.icon(
